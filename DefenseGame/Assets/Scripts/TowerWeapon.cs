...
public class TowerWeapon : MonoBehaviour
{
    ...
    private Tile ownerTile;    //현재 타워가 배치되어 있는 타입

    private float addedDamage; //버프에 의해 추가된 데미지
    private int buffLevel; //버프를 받는지 여부 설정(0:버프x 1-3 : 받는 버프 레벨)

    public Sprite TowerSprtie => towerTemplate.weapon[level].sprite;
    ...
    private Transform FindClosestAttackTarget()
    {
        ...
        float clossesDistSqr = Mathf.Infinity;
        //EnemySpawner의 EnmeyList에 잇는 현재 맵에 존재하는 모든 적 검사
        for(int i = 0; i<enemySpawner.EnemyList.Count; ++i)
        {
            float distance = Vector3.Distance(enemySpawner.EnemyList[i].transform.position, transform.position);
            //현재 검사중인 적과의 거리가 공격범위 내에 있고, 현재까지 검사한 적보다 거리가 가까우면
            if(distance <= towerTemplate.weapon[level].range && distance <= clossesDistSqr)
            {
                clossesDistSqr = distance;
    ...
    private void SpawnProjectile()
    {
        ...
        //생성된 발사체에게 공격대상(attackTarget) 정보 제공, 기본 공격력 + 버프에 의해 추가된 공격력
    ...
    private void SpawnLaser()
    {
        ...
        //같은 방향으로 여러개의 광선을 쏴서 그 중 현재 attackTarget과 동일한 오브젝트를 검출
        for (int i = 0; i < hit.Length; ++i)
        {
            ...
            //적 체력 감소(1초에 damage만큼 감소), 기본 공격력 + 버프에 의해 추가된 공격력
            float damage = towerTemplate.weapon[level].damage + AddedDamage;
    ...
    public bool Upgrade()
    {
        ...
        towerSpawner.OnBuffAllBuffTower();
        return true;
    }
